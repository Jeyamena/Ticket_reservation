...
top.maxsize(width=500 ,  height=600)
top.minsize(width=500 ,  height=600)
#bg = PhotoImage(file="1.jpg")
img = Image.open('C:/Users/Ace Training 13/Desktop/project/1.jpg')
bg=ImageTk.PhotoImage(img)
label1 = Label( top, image = bg)
label1.place(x = 0, y = 0)

# connecting to database
conn = mysql.connector.connect(host="localhost",user="root",password="",database="ticket_booking_database")
            
cursor = conn.cursor()
cursor.execute("CREATE TABLE IF NOT EXISTS ticket (name VARCHAR(30), ticket_id VARCHAR(50), ticket_date vARCHAR(50), ticket_validity VARCHAR(50))")
...
def Book():
    ...
    def BookNow():
        ...
        try:
            ...
            print(">>"+name.get())
            print("<<"+ticket_id.get())
            print("++"+ticket_date.get())
            print("<123>"+ticket_validity.get())
            
            cursor.execute("INSERT INTO ticket (name, ticket_id, ticket_date, ticket_validity) VALUES (%s, %s, %s, %s)", (str(name.get()), str(ticket_id.get()), str(ticket_date.get()), str(ticket_validity.get())))
            conn.commit()
